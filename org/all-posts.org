...
beyond trying to stitch a functional program with an imperative one in an abomination of nature.
...
(and in algorithmics in general), things are presented in an imperative fashion, the underlying
model usually being (based on) Turing machines. Persistent data structures are those for which,
even after modifying them, every "version" is still available, ie. every state of the data
structure between two mutable operations on them is still accessible as it was "back then", and
can be in turn operated onto as if nothing happened to it. This is very similar to how data
structures work in pure functional programming languages, because in this setting data structures
cannot be mutated, so if one still has a pointer to a certain version of a data structure, the
underlying data must be as it was when it was created; and it can freely be used without fear of
breaking someone else's data. However, it should to be noted that persistent data structure in
general need not to be implemented using immutability. They should just exhibit observational
immutability.
